...
class ScatterLayer:
    ...
    def __init__(self, data_source):
        ...
        self._base_marker_size = 6
        self.color_non_selected = 'gray'
        ...
        idx = ScatterLayer._color_counter % len(ScatterLayer._DEFAULT_COLORS)
        self._selection_color = ScatterLayer._DEFAULT_COLORS[idx]
        ScatterLayer._color_counter += 1
    ...
    def _update_plot(self):
        ...
        raw = self.data_source.get()
        if raw.shape[0] == 0 or raw.shape[1] < 2:
            self._scatter_base.setData(x=[], y=[], size=0)
            return
    ...
    def set_base_marker_size(self, size: int):
        self._base_marker_size = max(1, int(size))
        if self._scatter_base is not None:
            self._scatter_base.setSize(self._base_marker_size)
        self._update_plot()

    def set_color_non_selected(self, color: str):
    ...
    def get_selection_data(self):
        ...
        if data.shape[0] == 0 or idx.size == 0:
            return np.empty((0, data.shape[1])) if data.ndim == 2 else np.empty((0,))
        return data[idx]
    ...
    def clone(self):
        dup = ScatterLayer(self.data_source)
        dup.set_transform(self.transform)
        dup.set_base_marker_size(self._base_marker_size)
        dup.set_color_non_selected(self.color_non_selected)
